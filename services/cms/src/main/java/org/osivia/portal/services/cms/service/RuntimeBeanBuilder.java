...
public class RuntimeBeanBuilder {
    ...
    public void buildNamespaces() {
        ...
        for (Object propertyName : properties.keySet()) {
            if (propertyName instanceof String) {
                String sPropertyName = (String) propertyName;
                if (sPropertyName.startsWith(OSIVIA_CMS_SERVICE_INTEGRATION_PREFIX)
                        && sPropertyName.endsWith(OSIVIA_CMS_SERVICE_INTEGRATION_BEAN_NAME_SUFFIX)) {
                    String integrationServiceName = sPropertyName.substring(OSIVIA_CMS_SERVICE_INTEGRATION_PREFIX.length(),
                            sPropertyName.indexOf(OSIVIA_CMS_SERVICE_INTEGRATION_BEAN_NAME_SUFFIX));
                    String beanName = System
                            .getProperty(OSIVIA_CMS_SERVICE_INTEGRATION_PREFIX + integrationServiceName + OSIVIA_CMS_SERVICE_INTEGRATION_BEAN_NAME_SUFFIX);
                    String className = System
                            .getProperty(OSIVIA_CMS_SERVICE_INTEGRATION_PREFIX + integrationServiceName + OSIVIA_CMS_SERVICE_INTEGRATION_CLASSNAME_SUFFIX);
    ...
    public void buildPortalBeans(String repositoryName) {
        ...
        BeanDefinitionBuilder mapper = BeanDefinitionBuilder.rootBeanDefinition(DefaultPortalObjectPathMapper.class).setScope(BeanDefinition.SCOPE_SINGLETON)
         ...
         * </bean>
         */
        BeanDefinitionBuilder portalObjectFactory = BeanDefinitionBuilder.rootBeanDefinition(PortalObjectURLFactory.class)
                .setScope(BeanDefinition.SCOPE_SINGLETON).addPropertyValue("Namespace", repositoryName).addPropertyValue("Path", "/" + repositoryName)
         ...
         * </bean>
         */
        BeanDefinitionBuilder portalCommandFactory = BeanDefinitionBuilder.rootBeanDefinition(PortalObjectCommandFactory.class)
                .setScope(BeanDefinition.SCOPE_SINGLETON).addPropertyReference("Mapper", "portal:service=PortalObjectPathMapper,type=" + repositoryName);
         ...
         * </bean>
         */
        BeanDefinitionBuilder portalCommandFactoryDelegate = BeanDefinitionBuilder.rootBeanDefinition(CommandFactoryDelegate.class)
                .setScope(BeanDefinition.SCOPE_SINGLETON).addPropertyValue("Path", "/" + repositoryName)
